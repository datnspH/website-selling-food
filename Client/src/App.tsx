...
function App() {
  ...
  const Remove = (id: any) => {
    if (confirm("Are you sure you want to remove this")) {
      Delete(id).then(() => GetAll().then(({ data }) => setdata(data))).then(() => alert("Xóa Thành công!"))

    }
  }
  const RemoveCategory = (id: any) => {
    if (confirm("Are you sure you want to remove this")) {
      removeCategory(id).then(() => GetAllCategory().then(({ data }) => setcategory(data))).then(() => toast.error("Xóa Thành công!"))
      // removeCategory(id).then(() => setcategory(category.filter((data:any) => data._id !== id)))
    }
  }
  const Update = (data: any) => {
    UpdateNewProduct(data).then(() => GetAll().then(({ data }) => setdata(data)))
    alert("Cập nhật thành công!")
    // Window.prototype.location = "http://localhost:5173/Admin/Show";
  }
  const AddList = async (data1: any) => {
    const {data}:any = await CreatCategory(data1).then(() => GetAllCategory().then(({ data }) => setcategory(data))).then(() => toast.success(data.message))
